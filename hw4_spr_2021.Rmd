...
The relationship of the statistically significant variables with respect to our dependent variable HD is as follows:
  _AGE: for every 1 year increase in age, there is on average a .06 increase in likelihood of having HD 
  _SEX: for SEX=1 (Males), there is on average a .91 increase in the likelihood of having HD
  _SBP: for every 1 point increase in SBP, there is on average a .016 increase in the likelihood of having HD
  _CHOL: for every 1 point increase in CHOL, there is on average a .004 increase in the likelihood of having HD
  _CIG: for CIG=1 (is a smoker), there is on average a .012 increase in the likelihood of having HD
...
@chris @charles please check this: On an AUC basis, fit2 has a larger AUC than fit2. However, at certain thresholds, the ROC curve for fit2 appears to be below fit1. No, one curve does not always contain the other curve, and the ACU of one curve is not always larger than the AUC of the other one. This is because adding additional variables does not necessarily increase predictive power, particularly at specific thresholds. 
...
iii.  Estimate the Positive Prediction Values and Negative Prediction Values for `fit1` and `fit2` using .5 as a threshold. Which model is more desirable if we prioritize the Positive Prediction values? @chris @charles 
...
iii.  How would you classify Liz under this classifier? @chris @charles
